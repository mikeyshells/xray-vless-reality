...
# 官方 install-release.sh 更新后会重置 service 为 nobody, 这里恢复为 root
...
update_xray() {
    ...
    echo
    echo -e "$yellow更新 Xray 内核$none"
    echo "----------------------------------------------------------------"
    old_version=$(get_xray_version)
    echo -e "当前版本: ${cyan}${old_version}${none}"
    echo
    echo -e "将通过官方脚本安装最新版本, 并同步更新 geodata."
    echo -e "配置文件 ${cyan}/usr/local/etc/xray/config.json${none} 不会被覆盖."
    echo
    read -p "$(echo -e "确认更新? [${green}Y${none}/${red}n${none}]: ")" confirm
    case "$confirm" in
    "" | [Yy] | [Yy][Ee][Ss])
        ;;
    *)
    ...
    echo
    echo -e "$yellow正在安装最新 Xray 内核...$none"
    if ! bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install; then
        echo -e "$red Xray 内核更新失败.$none"
        echo "----------------------------------------------------------------"
        return 1
    fi

    echo
    echo -e "$yellow正在更新 geodata...$none"
    bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install-geodata

    echo
    echo -e "$yellow恢复服务运行用户为 root...$none"
    fix_xray_service_user

    run_service restart

    new_version=$(get_xray_version)
    echo
    echo -e "$green Xray 内核更新完成.$none"
    echo -e "更新前: ${cyan}${old_version}${none}"
    echo -e "更新后: ${cyan}${new_version}${none}"
    echo "----------------------------------------------------------------"
}
...
show_menu() {
    ...
    echo -e "  ${green}6$none) 更新 Xray 内核"
...
main() {
    if [[ $EUID -ne 0 ]]; then
        warn "请使用 root 权限运行此脚本, 例如: sudo bash xray-service.sh"
        exit 1
    fi
    ...
    while true; do
        show_menu
        read -p "$(echo -e "请输入序号 [0-6]: ")" choice
        case "$choice" in
        1)
            run_service start
            pause
